...
void createTextFile() { //Функція для створення початкового файлу
    ofstream fout("startTextFile.txt", ios::app); //Створення та відкриття файлу для запису
...
vector<string> sortLines(vector<string> lines) { //Функція для сортування рядків вектора
    if (!lines.empty()){ //Перевірка чи вектор містить елементи
        ...
        for (auto & line : lines) { //Цикл по рядках
            line += "(" + to_string(line.length()) + ")"; //Присвоювання довжини рядка
        }
    }
